...
export default function PartenairesHero() {
  ...
  return (
    <section ref={sectionRef} className="min-h-screen flex items-center pt-24 pb-16 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="absolute z-[1] rounded-full pointer-events-none" style={{ width: "280px", height: "280px", bottom: "-8%", left: "36%", background: "rgba(233,30,140,0.22)", filter: "blur(2px)" }} />
      <div className="absolute z-[1] rounded-full pointer-events-none" style={{ width: "220px", height: "220px", bottom: "-14%", left: "42%", background: "rgba(233,30,140,0.22)", filter: "blur(2px)" }} />

      {/* Image personnages — coin droit, pleine hauteur */}
      <img
        src="/partenaire/check_partenaire.webp"
        alt="Partenaires Imoni"
        className="absolute bottom-0 right-0 z-10 object-contain object-bottom pointer-events-none"
        style={{ height: "100%", width: "auto", maxWidth: "55%" }}
      />

      {/* Carte notification flottante */}
      <div className="absolute top-[42%] right-[16%] z-20 bg-white/90 backdrop-blur-sm rounded-2xl p-4 w-[220px]" style={{ boxShadow: "0 8px 32px rgba(0,0,0,0.12)", border: "1px solid rgba(233,30,140,0.1)" }}>
      ...
      <div className="relative z-10 w-full pl-16 sm:pl-24 lg:pl-36 pr-4">
        <div className="grid lg:grid-cols-[3fr_2fr] gap-8 items-center">

          {/* Gauche — texte */}
          <div className="space-y-6">
            <div>
              <span className="inline-block border border-gray-300 text-[11px] px-4 py-1.5 rounded-full tracking-[0.18em] uppercase text-gray-600 font-medium bg-white/70">
                Espace partenaires
              </span>
            </div>

            <div>
              <h1 className="text-6xl lg:text-7xl font-extrabold text-[#0D0F1A] leading-[1.05]">
                Devenez partenaire<br /><span className="text-[#E91E8C]">Imoni.</span>
              </h1>
              <p className="text-xl font-semibold text-[#0D0F1A] mt-5 leading-snug">
